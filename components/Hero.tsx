...
const Hero: React.FC<{ heroData: HeroData }> = ({ heroData }) => {
  ...
  const textColor = `rgba(${heroData.textColor?.rgb.r}, ${heroData.textColor.rgb.g}, ${heroData.textColor.rgb.b}, ${heroData.textColor.rgb.a})`;
  console.log(textColor);
  return (
    <div
      className={` ${textColor}  max-w-[1240px] mb-40 h-screen flex flex-col justify-center items-center p-4 rounded-lg`}
    >
      <div className=' flex flex-col justify-center items-center text-center sm:items-start space-y-6'>
        ...
        <div
          ...
          className={` m-4 absolute inset-0 flex flex-col text-center p-20 small:text-left small:p-32 ${positionClass}`}
        >
          ...
          <button className='flex items-center justify-center w-40 h-12 bg-primary-500 rounded-full text-[#FBF4E2] text-base-regular font-bold drop-shadow-md shadow-black'>
            Shop Now
          </button>
        </div>
        <Link href='/menu'>
